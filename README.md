...
- 中英分段接力播放（v2.8.0）：播报文本先按语言拆段——**中文段用 Chrome 内置
  `chrome.tts` 系统中文语音，英文段用 Edge-TTS 的 Ava 多语言语音**（发音纯正），
  按顺序接力读完。原因：公共 Edge-TTS worker 的中文合成路径已损坏（纯中文直接
  报错、混合文本被截断成首词碎片，本地 Whisper 转写实证），而纯英文路径一直稳定。
  语速对两个引擎都生效；系统语音自动挑选最优中/英文声源。
- 新播报会接管当前播放（打断旧接力链）；多标签去重仍然有效。
