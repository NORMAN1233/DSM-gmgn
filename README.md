...
- 官方钱包监控：GMGN 新弹出的钱包交易通知会记录对应通知行，不读写剪贴板。
  在扩展面板开启「钱包快捷跳转」，在当前 GMGN 页面按单键 `C`，直接点击最新通知行，由 GMGN 自身处理页面切换。
  按键瞬间重新识别当前最上方可见通知，节点复用或快速滚动时直接使用当前行，不等待后台或监控回调。
  无需设置浏览器组合快捷键；输入框打字、中文输入和长按不会触发。原组合快捷键仍兼容。
  已显示的通知也可直接点击；没有可见通知时提示展开钱包监控。不会排队逐条打开所有 CA。
